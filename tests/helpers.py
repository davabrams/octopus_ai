...
That is shorter than the old `p = GameParameters.copy(); p['x_len'] = 30`
AND it is typed underneath - plus a misspelled key raises instead of
silently doing nothing, which `p['x_lenn'] = 30` never did.
...
from OctoConfig import TEST, from_game_parameters, to_game_parameters

# The flat key surface, derived from the schema itself rather than
# hand-listed, so it cannot drift.
VALID_KEYS = frozenset(to_game_parameters(TEST))
...
def make_config(**overrides) -> Config:
    ...
    if unknown:
        raise UnknownConfigKey(
            ...
            f"Valid keys are the flat GameParameters surface; see "
            f"config_schema.py for the typed structure."
        )
    flat = {**to_game_parameters(TEST), **overrides}
    return from_game_parameters(flat)
...
def make_flat(**overrides) -> dict:
    ...
    return to_game_parameters(make_config(**overrides))
