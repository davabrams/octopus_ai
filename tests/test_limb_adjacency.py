...
class TestLimbAdjacencyRadius(unittest.TestCase):
    ...
    def test_find_color_uses_adjacency_radius_not_agent_range(self):
        ...
        self.assertTrue(
            all(r == self.params['adjacency_radius'] for r in seen),
            f"expected every call at adjacency_radius="
            f"{self.params['adjacency_radius']}, got {set(seen)}")
        self.assertNotIn(self.params['agent_range_radius'], seen)
