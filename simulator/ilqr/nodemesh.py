...
def main():
    """Interactive node-graph octopus animation.

    Runs a ~300 frame matplotlib animation where limb-tip nodes chase a
    moving attractor (plus the mouse pointer, if moved over the figure).
    Previously this ran at module import time; it is now guarded so the
    module is safely importable.
    """
    plt.ion()
    ...
    for val in np.arange(0, 30.0, 0.1):
        ...
        if mouse_pos["x"] and mouse_pos["y"]:
            attractors += [State(mouse_pos["x"], mouse_pos["y"])]
        ...
        def gen_cost_array(attractor_array: List):
            return [
                AllCosts(
                    ...
                    neighbor_states=[
                        node_array[n].state for n in node.neighbors
                    ],
                    max_distance=0.3,
                    min_distance=0.1,
        ...
        def line_search():
            # AllCosts.line_search returns (best_alpha, best_cost, best_grad)
            gradient_list = []
            alpha_list = []
            for c in cost_array:
                alpha, _cost, grad = c.line_search()
